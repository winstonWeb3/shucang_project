...
export default {
  ...
  getFieldsValue () {
    ...
    // 格式化categoryIds
	const goodsInfos = this.formData.debList
    goodsInfo.categorys = this.formatCategoryIds(goodsInfo.categoryIds)
	goodsInfos.debLists = this.formatdebLists(goodsInfos)
    // 商品基本数据
    const goodsFormData = _.pick(goodsInfo, [
      'goods_name', 'categorys','debLists', 'goods_no', 'delivery_type', 'sort',
      'delivery_id', 'status', 'spec_type', 'deduct_stock_type', 'content',
      'selling_point', 'serviceIds', 'sales_initial', 'is_points_gift',
      'is_points_discount', 'is_enable_grade', 'is_alone_grade','first','fixed_stock'
    ])
    // 单规格数据
  ...
  /**
   * 格式化debLists (用于表单元素选中)
   * @param {*} categoryIds
   */
  formatdebLists (debList) {
    return debList.map(id => { return { value: id } })
  },
  ...
  getCategoryList () {
	...
	getDebList () {
	  return new Promise((resolve, reject) => {
	    CategoryModel.getDeb()
	      .then(list => {
			  console.log('opo',list)
	        this.formData.debList = list
			console.log('opo',this.formData.debList)
	        resolve()
	      })
	  })
	},
