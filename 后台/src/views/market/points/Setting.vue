...
<!--        <a-divider orientation="left">积分抵扣</a-divider>
		...
		<a-form-item label="积分抵扣盲盒抽取次数(1次)" :labelCol="labelCol" :wrapperCol="wrapperCol">
		  <a-input-number
		    :min="1"
		    :max="1000"
		    :precision="1"
		  ...
		  <div class="form-item-help">
		    <p class="extra">注：抵扣个数请填写整数字0~1000，例：100个，即100个积分抵扣抽取盲盒一次</p>
		  </div>
		</a-form-item>
    ...
    */
    onFormSubmit (values) {
      ...
      Api.update(this.key, { form: values })
		...
		if(this.intergral){
			Api.boxInt({'ints':this.intergral}).then(res=>{
				console.log('asd',res)
			}).catch(ret=>{
