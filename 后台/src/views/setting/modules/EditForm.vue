<template>
  <a-modal
    ...
    @cancel="handleCancel"
  >
 <div style="width: 100%;height: 100%;padding: 20px 30px;display: flex;flex-wrap: wrap;" v-if="categoryList">
 	<div style="padding: 20px;" v-for="(item,index) in categoryList" @click="change(index)">
		<a-radio-group name="radioGroup" :default-value="0" v-model="form.zhangqi">
		   <a-radio :value="changeIndex == index? 0 :1" style="margin-left: 29px;margin-bottom: 10px;"></a-radio>          
...
export default {
  ...
  methods: {
     ...
     */
    handleSubmit (e) {
		this.$emit('result', this.categoryList[this.changeIndex])
		// 关闭对话框事件
		this.handleCancel()
		// console.log('asddf',this.categoryList,this.userArr)
		// const values = {
		// 	userIds:this.userArr,
		// 	goodsId:this.categoryList[this.changeIndex].goods_id,
