          ...
          <div class="tab-pane" v-show="tabKey == 0">
           ...
           <a-form-item label="藏品分类" :labelCol="labelCol" :wrapperCol="wrapperCol">
              ...
              <div class="form-item-help">
                <router-link target="_blank" :to="{ path: '/fragments/category/index' }">去新增</router-link>
                <a href="javascript:;" @click="onReloadCategoryList">刷新</a>
              </div>
            </a-form-item>
			<a-form-item label="合成所需碎片" :labelCol="labelCol" :wrapperCol="wrapperCol">
				<a-tree-select placeholder="请选择碎片"
					:dropdownStyle="{ maxHeight: '500px', overflow: 'auto' }"
					:treeData="formData.debList" treeCheckable treeCheckStrictly allowClear
					v-decorator="['debLists', {rules: [{required: true, message: '请至少选择1个碎片'}]}]">
				</a-tree-select>
				<!-- <div class="form-item-help">
					<router-link target="_blank" :to="{ path: '/fragments/category/index' }">去新增</router-link>
					<a href="javascript:;" @click="onReloadCategoryList">刷新</a>
           ...
           <!-- <a-form-item label="藏品编码" :labelCol="labelCol" :wrapperCol="wrapperCol">
			...
			<a-form-item label="开售时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
			   <a-date-picker @change="changeTime" v-model="startTime" showTime format="YYYY-MM-DD HH:mm:ss" style="width: 100%" placeholder="请输入日期"/>
			   </a-form-item>
						<!-- <a-form-item label="开售时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
 ...
 changeTime(value){
    ...
    handleSubmit (e) {
      ...
      validateFields((errors, values) => {
		...
		values.debrisIds = values.debLists.map(item => item.value)
		delete values.debLists
        // 整理商品分类ID集
