<template>
	<a-card :bordered="false">
			...
			<a-form :form="form" @submit="handleSubmit" :selfUpdate="true">
				...
				<div class="tabs-content">
					...
					<div class="tab-pane" v-show="tabKey == 0">
						...
						<!-- <a-form-item label="藏品分类" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-tree-select
                placeholder="请选择藏品分类"
              ...
              <div class="form-item-help">
                ...
                <a href="javascript:;" @click="onReloadCategoryList">刷新</a>
						...
						<a-form-item label="藏品图片" :labelCol="labelCol" :wrapperCol="wrapperCol"
							...
							<SelectImage multiple :maxNum="1" :defaultList="formData.goods.goods_images"
								v-decorator="['imagesIds', { rules: [{ required: true, message: '请至少上传1张藏品图片' }] }]" />
						</a-form-item>
              ...
              <div class="form-item-help">
                ...
                <a href="javascript:;" @click="onReloadDeliveryList">刷新</a>
						...
						<div style="width: 100%;height: 100%;padding: 20px 30px 60px 120px;display: flex;flex-wrap: wrap;"
							v-if="arr!=''">
							<div style="padding: 20px;" v-for="(item,index) in arr" :key="index">
								<div style="width: 80px;height: 80px;">
									<img style="width: 100%;height: 100%;" :src="item.goods_image" />
								</div>
								<div
									style="color: #000;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;max-width:76px;text-align: center;">
									{{item.goods_name}}
								</div>
				...
				<a-input placeholder="请输入限购数量" v-model="goods_purchase"/>
					...
					<div class="tab-pane" v-show="tabKey == 1">
						<a-form-item label="规格类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
							<a-radio-group v-decorator="['spec_type', {initialValue: 10, rules: [{ required: true }]}]"
						...
						<div v-show="form.getFieldValue('spec_type') == 20">
							<MultiSpec ref="MultiSpec" :defaultSpecList="formData.goods.specList"
						...
						<div v-show="form.getFieldValue('spec_type') == 10">
							<a-form-item label="藏品价格" :labelCol="labelCol" :wrapperCol="wrapperCol"
								...
								<a-input-number :min="0.01" :precision="2"
									v-decorator="['goods_price', { rules:[{ required: true, message: '请输入藏品价格' }] }]" />
              ...
              >
                ...
                <span class="ml-10">元</span>
							...
							<a-form-item label="总库存数量" :labelCol="labelCol" :wrapperCol="wrapperCol"
								...
								<a-input-number :min="0" :precision="0"
									v-decorator="['fixed_stock', { rules:[{ required: true, message: '请输入库存数量' }] }]" />
								<span class="ml-10">件</span>
							</a-form-item>
							<!-- <a-form-item
                label="藏品重量"
		...
		<EditForm ref="EditForm" @result="newResult" :categoryList="shopArr" />
	</a-card>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			// getPriceNum(){   //停用
			// 	GoodsApi.getPriceNum({'goods_id':this.$route.query.goodsId}).then(res => {
			// 		console.log(res)
			// 		this.goods_purchase = res.data.sum
			// 	})
			// },
			change() {
			...
			async initData() {
				...
				// 获取form所需的数据
				this.isLoading = true
				await GoodsModel.getFromData(this.goodsId)
					.then((res) => {
						// 商品表单数据
						if (!isEmptyObject(this.form.getFieldsValue())) {
							this.form.setFieldsValue(GoodsModel.getFieldsValue())
						}
						console.log('asff', this.formData.goods.first_goods_id)
						this.isLoading = false
						this.startTime = this.formData.goods.startTime
						this.author = this.formData.goods.author
						this.goods_address = this.formData.goods.goods_address
						this.logo = this.formData.goods.logo
						this.type = this.formData.type
						this.file_path = this.formData.file_path
						this.souTime = this.formData.goods.first_time
						this.goods_purchase = this.formData.goods.goods_purchase

						// this.fileList = this.fileList.push(this.formData.file_path)
					})
				await Api.getList().then(res => {
					console.log('llssdfl', res)
					console.log('lkj', res.data.list)
					var shopArr = res.data.list
					if (this.formData.goods.first == 1) {
						shopArr.forEach((itx, idx) => {
							itx.show = false
							for (var i in this.formData.goods.first_goods_id) {
								if (itx.goods_id == this.formData.goods.first_goods_id[i]) {
									itx.show = true
									this.arr.push(itx)
								}
							}
