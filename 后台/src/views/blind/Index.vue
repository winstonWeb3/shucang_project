  ...
  <a-card :bordered="false">
    ...
    >
      ...
      <span slot="goods_name" slot-scope="text">
        <p class="twoline-hide" style="width: 180px;">{{ text }}</p>
      </span>
	  <!-- 盲盒几率 -->
	  <span slot="probability" slot-scope="text">
	    <p class="twoline-hide" style="width: 120px;">{{ text }}</p>
	  </span>
		<!-- 商品类型 -->
		<span slot="type" slot-scope="text">
		  <p class="twoline-hide" style="width: 90px;">{{ text == 0?'普通藏品':'碎片藏品' }}</p>
		</span>
      <!-- 商品状态 -->
      ...
      <div class="actions" slot="action" slot-scope="text, item">
        <router-link :to="{ path: '/blind/update', query: { goodsId: item.goods_id } }">编辑</router-link>
...
export default {
  ...
  data () {
    return {
      ...
      loadData: param => {
        return GoodsApi.blind({ ...param, ...this.queryParam })
          .then(response => {
  ...
  methods: {
    ...
    handleUpdateStatus (goodsIds, state = true) {
      ...
      this.isLoading = true
      GoodsApi.blindState({ goodsIds, state })
        .then(result => {
          // 显示成功
