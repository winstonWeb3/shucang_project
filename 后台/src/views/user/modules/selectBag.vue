<template>
 ...
 <div style="width: 100%;height: 100%;padding: 20px 30px;display: flex;flex-wrap: wrap;">
 	<div style="padding: 20px;" v-for="(item,index) in bagList" @click="change(index)">
		<a-radio-group name="radioGroup" :default-value="0">
 		...
 		<div style="width: 80px;height: 80px;">
 			<img style="width: 100%;height: 100%;" :src="'https://123.8bcm.cn/uploads/' + item.file_path"/></div>
...
export default {
  ...
  methods: {
    change(index){
		console.log('asd',this.bagList)
		this.bagList[index].show = !this.bagList[index].show
	},
    /**
     * 显示对话框
     */
     ...
     */
    handleSubmit (e) {
		console.log('asddf',this.bagList,this.userArr)
		let arr = ''
		for(var i in this.bagList){
			if(this.bagList[i].show){
				arr += this.bagList[i].coll_id + ','
			}
		}
		if(arr == ''){
			return this.$message.success('请选择藏品', 1.5)
		}
		arr = arr.substring(0, arr.lastIndexOf(','));
		console.log('asdssss',arr)
		...
		Api.dellColl(values )
		  .then((result) => {
		    ...
		    this.handleCancel()
		    // 通知父端组件提交完成了
		    this.$emit('handleSubmits', values)
		  })
		  .finally((result) => {
