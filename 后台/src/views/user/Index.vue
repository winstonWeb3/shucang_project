	...
	<a-card :bordered="false">
        ...
        <span v-else>--</span>
          ...
          <span class="c-p">{{ text }}</span>
				...
				<p>
					<!-- <span>积分：</span> -->
					<span class="c-p">{{ item.balance }}元</span>
				</p>
			</span>
			<!-- 实际消费金额 -->
			...
			<span class="actions" slot="action" slot-scope="item">
				<a title="会员背包" v-action:recharge @click="handleInvite(item)">背包</a>
		...
		<RechargeForm ref="RechargeForm" @handleSubmit="handleRefresh" />
		 ...
		 <selectBag ref="selectBag" :userId="userList" :bagList="bagList"  @handleSubmits="handleRefresh"/>
...
<script>
	...
	import selectBag from './modules/selectBag'
	import {
		STable
	} from '@/components'
	import {
	...
	export default {
		...
		methods: {
			...
			// 邀请列表
			handleInvite (item) {
				console.log('dkdk',item)
				this.userList = item
				GradeApi.coll({user_id:item.user_id}).then(res => {
					console.log('llssdfl', res)
					if(res.data!=''){
						...
						this.$refs.selectBag.edit(item)
					}else{
						this.$message.error('暂无藏品', 1.5)
					}
