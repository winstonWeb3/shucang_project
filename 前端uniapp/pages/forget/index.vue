<template>
	<view class="login">
		...
		<view class="box">
			...
			<!-- <view class="code_box">
				<view class="ipt"><input placeholder-style="font-size:16px;" type="text" value="" v-model="code"
						placeholder="短信验证码" />
				</view>
				<view class="code" @click="getCode">{{codeTip}}</view>
			</view> -->
			...
			<view class="flex">
				...
				<view style="width: 86%;">
					<input type="password" placeholder-style="font-size:16px;color:#BDBDBD;" v-model="password"  placeholder="请输入新密码" class="login-phone" />
				</view>
			</view>
			<!-- <input type="password" placeholder-style="color:#BDBDBD" v-model="password"  placeholder="请输入新密码" class="login-phone" /> -->
		</view>
		<!-- <view class="input-text">未注册的手机号通过验证将自动注册</view> -->
		<view class="login-submit" :class="subType ? 'login-sub':''" @click="loginSubmit">提交</view>
   ...
   //       if(this.getRequest().code){
		...
		methods: {
			...
			loginSubmit: tool.debounce(function() {
				...
				this.$request('POST', 'Passport/forgetPass', {
					phone: this.phone,
